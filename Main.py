class UVSim:
    def __init__(self, ):
        ...
        self.accumulator = None  # this is our accumulator. We want to use this.

    def process_instructions(self, input_text):
        # loads each instruction into the corresponding register
        for item in range(len(input_text)):
            if len(input_text[item]) != 5:
                # if it is greater than 5, then it is not something our program should recognize. we need
                ...
                try:
                    ...
                    if item <= 9:
                        ...
                        self.memory_dict[item] = [temp_bool, input_text[item][1:5]]
                        self.instruction_amount += 1
                except ValueError:
                    print(input_text[item], " is an invalid command! 31")
            else:
        ...
        for register in range(self.instruction_amount):
            if register <= 9:
                ...
                temp_instruction = self.memory_dict[register]
            if not temp_instruction[0]:
                pass
            else:
                our_instruction = temp_instruction[0:1]
                our_register = temp_instruction[2:3]
                if our_instruction == "10":  # call Read
                    ...
                    self.memory_dict[register][0] = False
